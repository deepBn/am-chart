...
function CandleChart() {
    ...
    const width = useMemo(() => 1300, [])
    const height = useMemo(() => 700, [])
    const margin = useMemo(() => {
        return { left: 0, right: 72, top: 0, bottom: 24 }
    }, [])
    ...
    const { data, xScale, xAccessor, displayXAccessor } = useMemo(() => ScaleProvider(
        initialData ?? []
    ), [])
    const pricesDisplayFormat = useMemo(() => format(".2f"), [])
    const max = useMemo(() => xAccessor(data[data.length - 1]), [])
    const min = useMemo(() => xAccessor(data[Math.max(0, data.length - 100)]), [])
    const xExtents = useMemo(() => [min, max + 5], [])
